...
export default function ServicesCarousel({ services, autoMs = 3000 }: { services: Service[]; autoMs?: number }) {
  const pages = useMemo(() => {
    const arr: Service[][] = [];
    for (let i = 0; i < services.length; i += 2) arr.push(services.slice(i, i + 2));
    return arr;
  }, [services]);
  ...
  const translatePct = (pageIndex * 100) / numPages;

  return (
    <div className="relative overflow-hidden">
      <div className="flex transition-transform duration-700 ease-in-out" style={{ width: `${numPages * 100}%`, transform: `translateX(-${translatePct}%)` }}>
        {pages.map((page, i) => (
          <div key={i} className="w-full shrink-0 px-0 sm:px-1" style={{ width: `${100 / numPages}%` }}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {page.map((svc, j) => (
                <div key={j} className="rounded-2xl border border-black/10 dark:border-white/10 p-6 sm:p-8 hover:shadow-sm transition bg-background">
                  <h3 className="text-xl sm:text-2xl font-semibold tracking-tight">{svc.title}</h3>
                  <p className="mt-2 text-sm sm:text-base text-black/70 dark:text-white/70">{svc.description}</p>
                </div>
              ))}
            </div>
          </div>
